...
void MinIOUploader::connectToBucket(const Aws::String &endpoint, const Aws::String &keyId, const Aws::String &keySecret, const Aws::String &bucketName) noexcept(false){
...
void MinIOUploader::setBucketPolicy() noexcept(false){
    Aws::String policy = "{\n"
                         "     \"Version\":\"2012-10-17\",\n"
                         "      \"Statement\":[\n"
                         "          {\n"
    ...
    *request_body << policy;
    ...
    if (!outcome.IsSuccess()) {
        throw MinioException("Errore: bucket policy");
    }
    else {
...
// dell'istanza di Miniouploader
void MinIOUploader::createBucket() noexcept(false){
    ...
    if(!outcome_head.IsSuccess()){
        ...
        if (!outcome.IsSuccess()) {
            throw MinioException("Errore: bucket creation");
...
void MinIOUploader::putImage(const Aws::String &key, const Aws::String &filename) noexcept(false){
    ...
    if (!*inputData) {
        ...
        throw MinioException("Errore: lettura file");
    }
    ...
    if (!outcome.IsSuccess()) {
        throw MinioException("Errore: caricamento nel bucket");
    }
    else {
